-- =============================================
-- Migration: V027__Ensure_Cloud_Storage_Objects
-- Author: System
-- Date: 2026-01-09
-- Description: Ensure DocumentData columns, DocumentShares table, and cloud storage SPs
-- =============================================

BEGIN TRANSACTION;

PRINT 'Applying migration V027: Ensure cloud storage objects...';
...
BEGIN
    ...
    IF COL_LENGTH('dbo.DocumentData', 'IsActive') IS NULL
...
BEGIN
    ...
    (
        @RelId, @RelCode, @DisplayText, @ValueFile, @Code, @DataType,
        (CASE WHEN @ParentId <= 0 THEN NULL ELSE @ParentId END), @IsFolder, @AccessLevel,
        @CreatedBy, GETDATE(), GETDATE(), 1, 0
    );
END
GO
...
PRINT 'Migration V027 completed successfully.';
